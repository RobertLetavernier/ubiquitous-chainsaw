...
if (!$bError)
{
    ...
    foreach ($asCodeXfurn as $sCodeXfurn)
    {
        ...
        $aoFichier = FichierDAO::getInstance()->findListByEltIdxListAndByFicNumIndexedOrTarification($aiEltIdx, 2, $g_iCateIdxTarification);
        foreach ($aoElement as $oElementModel)
        {
                    ...
                    foreach ($aoFichier[$oElementModel->getEltIdx()] as $oFichier)
                    {
                        ...
                        if (file_exists($sImagePath . "/" . $sFileName) && ($sFileName != ""))
                        {
                            ...
                            if ($oElementPrincipal != null && $oElementPrincipal->getEltIdx() == $oElementModel->getEltIdx())
                            {
                                ...
                                $asMainImage['original'] = $PathVirtuel . 'image2.php?fic=' . urlencode($asEnseigne[$g_iFrontEnsIdx] . "/" . CategorieHelper::getInstance()->getDossierByCateIdx($oElementModel->getEltCateIdx()) . '/' . $sFileName);
                            }
                            else
                            {
                                $asImage = array();
                                ...
                                $asImage['original'] = $PathVirtuel . 'image2.php?fic=' . urlencode($asEnseigne[$g_iFrontEnsIdx] . "/" . CategorieHelper::getInstance()->getDossierByCateIdx($oElementModel->getEltCateIdx()) . '/' . $sFileName);
                                $amImages[] = $asImage;
                            }
                        }
                    }
